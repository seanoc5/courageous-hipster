...
public class SearchConfigurationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all searchConfigurations")
                ...
                .exec(
                    http("Create new searchConfiguration")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_searchConfiguration_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created searchConfiguration").get("${new_searchConfiguration_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created searchConfiguration").delete("${new_searchConfiguration_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SearchConfiguration entity").exec(scn);
