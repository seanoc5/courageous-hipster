 ...
 */
public class TopicGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all topics").get("/api/topics").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new topic")
                        .post("/api/topics")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created topic").get("${new_topic_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created topic").delete("${new_topic_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Topic entity").exec(scn);
