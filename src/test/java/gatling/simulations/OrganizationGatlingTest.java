...
public class OrganizationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all organizations").get("/api/organizations").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created organization").get("${new_organization_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created organization").delete("${new_organization_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
