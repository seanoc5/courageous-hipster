...
public class ThingTypeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all thingTypes").get("/api/thing-types").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new thingType")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"label\": \"SAMPLE_TEXT\"" +
                                ", \"parentClass\": \"SAMPLE_TEXT\"" +
                                ", \"descrption\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_thingType_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created thingType").get("${new_thingType_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created thingType").delete("${new_thingType_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ThingType entity").exec(scn);
